...
def test_curriculum_authoring_is_role_guarded(monkeypatch):
...
def test_curriculum_draft_review_publish_flow_preserves_draft_isolation(monkeypatch):
    ...
    tutor = TestClient(_app_for_user({"sub": "tutor-1", "role": "tutor"}))
    admin_client = TestClient(_app_for_user({"sub": "admin-1", "role": "admin"}))

    created = tutor.post("/admin/curriculum/lessons/drafts", json=_draft_payload())
...
def test_curriculum_publish_rejects_unapproved_versions(monkeypatch):
    ...
    created = admin_client.post("/admin/curriculum/lessons/drafts", json=_draft_payload())
    version_id = created.json()["versionId"]
    published = admin_client.post(
        "/admin/curriculum/lessons/lesson-linear-ops/publish",
        json={"versionId": version_id, "expectedPublishedVersionId": None},
...
def test_curriculum_publish_uses_compare_and_set_pointer(monkeypatch):
    ...
    created = admin_client.post("/admin/curriculum/lessons/drafts", json=_draft_payload())
    version_id = created.json()["versionId"]
    admin_client.post(f"/admin/curriculum/lessons/lesson-linear-ops/drafts/{version_id}/submit-review")
    admin_client.post(f"/admin/curriculum/lessons/lesson-linear-ops/drafts/{version_id}/approve")
    published = admin_client.post(
        "/admin/curriculum/lessons/lesson-linear-ops/publish",
...
def test_curriculum_rollback_repoints_to_prior_version(monkeypatch):
    ...
    first = admin_client.post("/admin/curriculum/lessons/drafts", json=_draft_payload())
    first_version = first.json()["versionId"]
    admin_client.post(f"/admin/curriculum/lessons/lesson-linear-ops/drafts/{first_version}/submit-review")
    admin_client.post(f"/admin/curriculum/lessons/lesson-linear-ops/drafts/{first_version}/approve")
    admin_client.post(
        "/admin/curriculum/lessons/lesson-linear-ops/publish",
    ...
    second = admin_client.post("/admin/curriculum/lessons/drafts", json=second_payload)
    second_version = second.json()["versionId"]
    admin_client.post(f"/admin/curriculum/lessons/lesson-linear-ops/drafts/{second_version}/submit-review")
    admin_client.post(f"/admin/curriculum/lessons/lesson-linear-ops/drafts/{second_version}/approve")
    admin_client.post(
        "/admin/curriculum/lessons/lesson-linear-ops/publish",
