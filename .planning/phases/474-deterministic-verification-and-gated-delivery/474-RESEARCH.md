...
Acquire dependencies before the test subprocess is placed under network denial. Create two different temporary `UV_PROJECT_ENVIRONMENT` paths, run `uv sync --frozen --python 3.12 --extra dev`, then run identical `python -m pytest` commands with only the clock changed. Clear AWS variables, set `AWS_EC2_METADATA_DISABLED=true`, point shared credentials/config to nonexistent files, deny sockets, freeze time, and use a deterministic hash/test seed. [CITED: https://docs.astral.sh/uv/concepts/projects/sync/] [CITED: https://pypi.org/project/pytest-socket/] [CITED: https://time-machine.readthedocs.io/en/latest/usage.html]
...
Run `npm ci`, `npm run lint`, `npm run build`, dependency policy, focused OpenAPI/backend contract checks, and Playwright through the same orchestrator. Configure zero retries for the release gate or add `--fail-on-flaky-tests`; parse JSON/JUnit output and fail on failed, flaky, skipped, or fixme tests. Playwright documents that tests passing only on retry are “flaky,” and skipped/fixme annotations do not execute normally. [CITED: https://playwright.dev/docs/test-retries] [CITED: https://playwright.dev/docs/test-annotations]
...
| Python network interception | Patch individual SDK calls | `pytest-socket` plus AWS env/metadata denial | Central socket denial catches unanticipated call paths; AWS denial handles credential discovery separately. [CITED: https://pypi.org/project/pytest-socket/] |
...
| OS-registered state | No launchd/systemd/pm2/task registrations are part of the release path. Docker CLI exists but its daemon is unavailable. [VERIFIED: repository search and environment probe] | None for OS registration. Keep formal local verification independent of Docker; run arm64 artifact boot smoke in a capable CI environment. |
...
**How to avoid:** acquire first; run tests in a scrubbed subprocess; use `time-machine`, socket denial, metadata disablement, nonexistent AWS config paths, and intentional self-tests that would fail if real clock/network/AWS access succeeds. [VERIFIED: D-04]
...
| Docker | arm64 Lambda boot smoke locally | Partial | CLI 29.6.1; daemon unavailable | Run artifact-compatible smoke in a capable CI runner; local receipt is exact NOT RUN for that external obligation. [VERIFIED: Docker probe and D-03] |
...
**Missing dependencies with fallback:** local Docker daemon is replaced by CI artifact smoke; GitHub Actions emulation is replaced by script/workflow-contract tests. [VERIFIED: environment probe]
