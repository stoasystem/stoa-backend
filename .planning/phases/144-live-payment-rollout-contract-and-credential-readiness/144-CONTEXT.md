...
v3.9 completed the local Stripe-first payment provider MVP: checkout session creation, provider billing status, signed webhook lifecycle, parent payment UX, and admin billing visibility. `stoa_docs` still calls for real payment-provider rollout, Stripe-backed TWINT inclusion, invoices/receipts/refunds, tax/accounting, and dunning. Phase 144 defines the live rollout contract before code changes or provider configuration are changed.
...
- How should Stripe-backed TWINT be gated, validated, and surfaced in the v4.4 rollout?
...
- Prefer Stripe-hosted Checkout, TWINT, invoice, refund, and dunning primitives before building custom billing automation.
