...
- [x] **UI-04**: Production admin browser smoke uses a real existing admin session or approved secret-backed credential path without creating temporary production admin accounts. Completed in Phase 36 production smoke.
- [x] **UI-05**: Production browser smoke is read-only by default, verifies route/API/auth/privacy behavior, redacts evidence artifacts, and does not mutate customer reports. Completed in Phase 36 production smoke.

### Runbook, Release Gate, and Live Verification

- [x] **OPS-01**: Report recovery runbook covers async job creation, preview review, cancellation, stop conditions, audit lookup, stalled job handling, SES/Lambda/DynamoDB observability, and escalation. Completed in Phase 37.
- [x] **OPS-02**: Release checklist captures Lambda build manifest, backend source provenance, Lambda CodeSha/source evidence, CDK diff/deploy evidence, and stale-dist guard results. Completed in Phase 37.
- [x] **OPS-03**: Live verification proves admin-only job/audit APIs, read-only production browser smoke, metadata-only privacy boundaries, and clean CDK diff/deploy state. Completed in Phase 37.
- [x] **OPS-04**: Final milestone audit records implementation evidence, research decisions, live verification outputs, residual risks, and deferred follow-up work. Completed in Phase 37.
...
*Last updated: 2026-06-05 after v1.6 milestone completion*
