...
3. One formal cross-repository gate deterministically verifies backend and Web source, builds each artifact once, deploys the exact release set to staging, and proves staging smoke plus automatic rollback through a controlled staging failure exercise. The complete production release control plane is deferred to a separate future phase or milestone.
...
- [ ] **V9QUAL-01:** Local formal verification and CI invoke one authoritative cross-repository entry point that creates a fresh Python 3.12 environment from committed `uv.lock` in frozen mode and a clean Web install from committed `package-lock.json`; neither CI nor a developer environment may substitute a weaker gate. *(Implemented by completed Phase 474 plans; independent current-candidate phase verification remains pending.)*
- [ ] **V9QUAL-02:** The complete Python suite passes twice in separate fresh environments, once at a fixed standard time and once at an explicit future fixed time, with deterministic seed/collection identity, ambient AWS credentials denied, non-allowlisted network denied, and exactly zero skip, xfail, or xpass outcomes; unavailable external checks are separate exact `NOT RUN` obligations and never count as passing. *(Implemented by completed Phase 474 plans; independent current-candidate phase verification remains pending.)*
- [ ] **V9QUAL-03:** The actual Web repository passes locked install, ESLint, TypeScript production build, dependency checks, and candidate-bound Web release-contract suites through the same formal gate. Phase 474 proves exact-source gate wiring and fail-closed result handling; component, accessibility/responsive, and real non-intercepted Playwright journey acceptance belong only to V9WEB-07/Phase 478 and publish separate receipts without blocking Phase 474 completion.
...
- [ ] **V9QUAL-06:** Phase 474 implements the staging-only minimum versioned cross-repository release infrastructure: exactly one GitHub Environment `staging` restricted to branch `main`, no tag deployments, no manual reviewer, main branch protection, one active main ruleset, `StoaReleaseStaging`, immutable artifact/evidence storage, immutable Lambda versions with staging aliases, versioned Web release objects and `/served-release.json`, durable release/rollback coordinates, and exactly three least-privilege roles named `StoaStagingDeployRole`, `StoaStagingReadRole`, and `StoaStagingRollbackRole`. All three roles trust only `repo:stoasystem/stoa-backend:environment:staging`; deploy/read/rollback permissions remain separate. One manifest binds exact backend/Web/infra commits, lock/source identities, runtimes, verification runs, and artifact digests; artifacts build once and deploy unchanged to staging. Separate `deploy-staging`, `smoke-staging`, and `rollback-staging` jobs share the `staging` Environment, and a controlled staging failure automatically restores the previously verified set. Failed and staging candidates are retained at least 90 days and current/known-good rollback sets indefinitely; every gate change runs intentional-failure/tamper tests. Production GitHub Environments, AWS stacks, IAM roles, OIDC trusts, deployment, smoke, rollback, reviewer policy, and promotion eligibility are `DEFERRED_OUT_OF_SCOPE` to a separate future phase or milestone.
- [ ] **V9QUAL-07:** Phase 473 evidence publication can be reverified from a later clean metadata HEAD by selecting the explicit candidate and its single direct publication commit, reading the four publication artifacts from immutable Git blobs, and proving the current HEAD descends from that publication without changing those blobs. *(Implemented by completed Phase 474 plans; independent current-candidate phase verification remains pending.)*

### Data Consistency And Concurrency

- [x] **V9DATA-01:** Question quota, idempotency, usage ledger, upload consumption, and initial question persistence commit atomically or converge through an explicitly tested recovery state.
- [x] **V9DATA-02:** Concurrent teacher takeover has exactly one winner, one session, and one notification through a conditional/transactional claim.
- [x] **V9DATA-03:** Parent/student forward and reverse bindings and required profile changes commit transactionally, and a reconciliation tool repairs historical asymmetry idempotently.
...
- [x] **V9DATA-07:** Notification delivery-begin distinguishes typed conditional/fence loss from transient DynamoDB/provider dependency failure; only proven account-deletion loss may terminalize as `canceled_account_deletion`, while transient failure remains recoverable and a healthy retry can deliver once.
- [x] **V9DATA-08:** An identical account-deletion retry after a lost successful response returns the stored completed `deleted` receipt through the real endpoint without reopening cleanup, requiring minimized-away branch proof, or producing a replay conflict.

### Billing And Paid Access

- [ ] **V9BILL-01:** Each Web checkout business request carries a required idempotency key that is reused by the backend, Stripe, and durable local command state, producing at most one active provider session. *(Administratively accepted through the Phase 476 project-owner manual waiver on 2026-07-30; independent verification remains incomplete.)*
- [ ] **V9BILL-02:** Provider success with local failure, local success with response timeout, duplicate browser retry, and delayed webhook all reconcile to one support-visible billing state. *(Administratively accepted through the Phase 476 project-owner manual waiver on 2026-07-30; independent verification remains incomplete.)*
- [ ] **V9BILL-03:** Checkout success/cancel URLs are parsed structurally and restricted to configured exact Web origins and approved paths for the current environment. *(Administratively accepted through the Phase 476 project-owner manual waiver on 2026-07-30; independent verification remains incomplete.)*
- [ ] **V9BILL-04:** A Stripe test-mode browser journey proves signed webhook processing changes parent/student effective entitlement and quota exactly once and remains explainable in parent/admin Web views. *(Administratively accepted through the Phase 476 project-owner manual waiver on 2026-07-30; repository-level provider/security evidence remains incomplete.)*

### Web Authentication And Contract Convergence
...
- [ ] **V9WEB-03:** Staging/production Web configuration and shared startup/service foundations fail closed, cannot enable demo fallback, static truth, placeholder success, or virtual success, and expose explicit loading, denied, expired-session, dependency-failure, and retry states. Phase 477 consumes the runtime configuration and startup barrier already implemented by Phase 474; route-by-route truth closure belongs only to V9WEB-10/Phase 478.
...
- [ ] **V9WEB-06:** A parent can sign in, see only bound children, inspect learning/usage/report/entitlement state, and consume the existing Phase 476 checkout, recovery, billing-state, and quota projections through the Web App. Phase 478 verifies Web integration and preserves the Phase 476 manual-waiver boundary; it does not recreate checkout, webhook, allowance, reminder, or admin-recovery logic.
...
- [ ] **V9INFRA-01:** After Phase 474 completes its release topology, a read-only retained-route inventory identifies the exact runtime-only infrastructure delta across Cognito, DynamoDB indexes, S3 lifecycle, queues, WebSocket resources, and backup/restore. Phase 479 defines or imports only gaps proven necessary by that inventory and observes the deferred real S3 multipart/version/promotion/restart and live DynamoDB behavior; release roles, immutable stores, Lambda aliases, Web pointers, protected environments, promotion, and rollback remain exclusively owned by Phase 474.
...
- [ ] **V9OPS-01:** Consuming V9PRIV-04's shared redaction evidence, liveness, dependency readiness, request/trace correlation, latency/error/business metrics, actionable alarms, deployed cleanup scheduling/retry evidence, and deployed log capture cover critical auth, question, billing, notification, and Web request paths without creating a second log-redaction contract.
- [ ] **V9OPS-02:** Audited practice, teacher, admin, and notification access paths use exact keys/indexes and complete pagination so first-page or hard-limit truncation cannot silently omit records. WebSocket pagination is implemented once under V9INFRA-02 and consumed here as evidence.
- [ ] **V9OPS-03:** Live provider-effect observation plus synthetic and staging browser/API probes exercise the critical Web release set, distinguish dependency degradation from process liveness, and retain correlation and alarm evidence. A controlled staging failure invokes and observes the exact Phase 474 rollback coordinator without rebuilding artifacts or implementing a second promotion/rollback mechanism; production smoke is `DEFERRED_OUT_OF_SCOPE` with the future production control plane.
...
- [ ] **V9CLOSE-02:** A final bounded review confirms that critical v9.0 auth, authorization, usage, billing, realtime, and Web contract changes have testable boundaries and records unrelated refactors in a prioritized debt register. A newly confirmed release-blocking boundary defect produces `HOLD` and a gap routed back to its owning phase; Phase 481 does not implement the fix or start a broad architecture rewrite.
- [ ] **V9CLOSE-03:** Final audit indexes, without rerunning valid same-candidate work, backend/Web/infra source SHAs, release-manifest and artifact digests, test/build/dependency results, the executable route inventory, all-role request/run IDs and redacted API/provider/browser evidence, finding disposition, staging and controlled rollback evidence, and an explicit early-testing, internal-only, beta, or hold decision. It records the production release control plane as `DEFERRED_OUT_OF_SCOPE` and does not require, accept, or synthesize production results.

## Definition Of Done

- All 51 requirements are completed with repository evidence and mapped to exactly one phase.
- Owner waivers are recorded separately and do not count as independently verified requirements; a final `HOLD` decision may preserve a waiver without fabricating missing evidence.
- Completed Phase 472, 473, and 475 requirements remain complete; their evidence is rechecked through the common release candidate rather than reopened or relabeled.
...
- One cross-repository manifest proves locked Web verification, integrated student, parent, teacher, and admin/operator browser journeys plus the complete route inventory, versioned staging infrastructure, staging smoke, and controlled automatic rollback for the same build-once release set; the production release control plane is recorded as `DEFERRED_OUT_OF_SCOPE`.
...
- Production release infrastructure, identities, approval, promotion, smoke, and rollback are absent from current phase scope and require a separately planned future phase or milestone; v9.0 planning does not authorize real charging, bulk notification, or broader rollout.
...
| V9PRIV-04 | Phase 480 | Partial — local controls exist; cross-repository retained-route proof pending |
| V9QUAL-01 | Phase 474 | Partial — implemented; phase verification missing |
| V9QUAL-02 | Phase 474 | Partial — implemented; phase verification missing |
| V9QUAL-03 | Phase 474 | Partial — Phase 474 incomplete |
| V9QUAL-04 | Phase 474 | Partial — Phase 474 incomplete |
| V9QUAL-05 | Phase 474 | Partial — Phase 474 incomplete |
| V9QUAL-06 | Phase 474 | Partial — Phase 474 incomplete |
| V9QUAL-07 | Phase 474 | Partial — implemented; phase verification missing |
| V9DATA-01 | Phase 475 | Complete |
| V9DATA-02 | Phase 475 | Complete |
| V9DATA-03 | Phase 475 | Complete |
| V9DATA-04 | Phase 475 | Complete |
| V9DATA-05 | Phase 475 | Complete |
| V9DATA-06 | Phase 475 | Complete |
| V9DATA-07 | Phase 475 | Complete |
| V9DATA-08 | Phase 475 | Complete |
| V9BILL-01 | Phase 476 | Owner-waived — independent verification incomplete |
| V9BILL-02 | Phase 476 | Owner-waived — independent verification incomplete |
| V9BILL-03 | Phase 476 | Owner-waived — independent verification incomplete |
| V9BILL-04 | Phase 476 | Owner-waived — provider evidence not independently reproduced |
| V9WEB-01 | Phase 477 | Pending — cross-repository contract missing |
| V9WEB-02 | Phase 477 | Pending — Web role/auth surface conflicts with backend |
| V9WEB-03 | Phase 477 | Partial — Phase 474 startup/config complete; reachable truth closure pending |
| V9WEB-04 | Phase 478 | Partial — backend primitives exist; Web journey incomplete |
| V9WEB-05 | Phase 478 | Partial — backend complete; Web contract incompatible |
| V9WEB-06 | Phase 478 | Partial — Phase 476 surfaces exist; parent integration proof pending |
| V9WEB-07 | Phase 478 | Partial — intercepted/demo browser coverage only |
| V9WEB-08 | Phase 478 | Partial — backend complete; Web teacher journey broken |
| V9WEB-09 | Phase 478 | Partial — broad UI exists; retained routes not integrated |
| V9WEB-10 | Phase 478 | Partial — manual route metadata exists; executable inventory missing |
| V9INFRA-01 | Phase 479 | Partial — base CDK exists; exact retained-route delta pending |
| V9INFRA-02 | Phase 479 | Partial — local service exists; deployed safe lifecycle missing |
| V9INFRA-03 | Phase 479 | Partial — client recovery exists; real transport proof missing |
| V9OPS-01 | Phase 480 | Partial — local pieces exist; complete staged coverage pending |
| V9OPS-02 | Phase 480 | Partial — several critical paths still truncate |
| V9OPS-03 | Phase 480 | Pending |
| V9CLOSE-01 | Phase 481 | Pending |
| V9CLOSE-02 | Phase 481 | Partial — existing boundaries/debt evidence require final reconciliation |
...
| Phase 474 | TEST-001, OPS-001, OPS-002, SEC-007, QUALITY-001 | Partial implementation; finish retained plans and independently verify |
| Phase 475 | DATA-001, BUG-002, DATA-003, BUG-006, BUG-004 | Verified complete |
| Phase 476 | DATA-002, SEC-008 | Owner-waived; independent provider/security verification incomplete |
...
| Phase 479 | FEATURE-002, OPS-003 | Close exact retained-route runtime deltas and browser realtime only |
| Phase 480 | SEC-006, PERF-001, OPS-004 | Close redaction, readiness/correlation, non-WebSocket pagination, alarms, and probes |
| Phase 481 | ARCH-001, ARCH-002, DOC-001; native-only FEATURE-001, BUG-003, BUG-005, TEST-002 | Reconcile truth/debt and decide; perform no feature work; native-only findings remain deferred |
