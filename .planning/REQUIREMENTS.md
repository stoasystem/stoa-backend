...
- [ ] **V9OPS-03:** Live provider-effect observation plus synthetic and staging browser/API probes exercise the critical Web release set, distinguish dependency degradation from process liveness, and retain correlation and alarm evidence. A controlled non-production failure invokes and observes the exact Phase 474 rollback coordinator without rebuilding artifacts or implementing a second promotion/rollback mechanism; production smoke remains separately authorized or exact `NOT RUN`.
...
- [ ] **V9CLOSE-03:** Final audit indexes, without rerunning valid same-candidate work, backend/Web/infra source SHAs, release-manifest and artifact digests, test/build/dependency results, the executable route inventory, all-role request/run IDs and redacted API/provider/browser evidence, finding disposition, staging and controlled non-production rollback evidence, and an explicit early-testing, internal-only, beta, or hold decision. Separately authorized production results are included when available; otherwise production promotion/smoke/rollback remain exact `NOT RUN`.
