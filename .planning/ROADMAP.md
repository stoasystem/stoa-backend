...
v5.10 and v5.11 closed the account-operations and usage-ledger polish chain. The remaining product queue now splits into two categories:
...
v5.12 should therefore build the curriculum tooling itself rather than start native apps or wait for external provider prerequisites.
...
- Implement frontend admin/tutor tooling against real API clients with no demo fallback for authoring/migration state.
